...
class BaseMapTest:
    ...
    def test_map_basics_2(self):
        h = self.Map()
        TestCase().assertEqual(len(h), 0)

        h2 = h.set('a', 'b')
        TestCase().assertIsNot(h, h2)
        TestCase().assertEqual(len(h), 0)
        TestCase().assertEqual(len(h2), 1)

        TestCase().assertIsNone(h.get('a'))
        TestCase().assertEqual(h.get('a', 42), 42)

        TestCase().assertEqual(h2.get('a'), 'b')

        h3 = h2.set('b', 10)
        TestCase().assertIsNot(h2, h3)
        TestCase().assertEqual(len(h), 0)
        TestCase().assertEqual(len(h2), 1)
        TestCase().assertEqual(len(h3), 2)
        TestCase().assertEqual(h3.get('a'), 'b')
        TestCase().assertEqual(h3.get('b'), 10)

        TestCase().assertIsNone(h.get('b'))
        TestCase().assertIsNone(h2.get('b'))
    ...
    def test_map_basics_3(self):
        ...
        h1 = h.set('1', o)
        h2 = h1.set('1', o)
        TestCase().assertIs(h1, h2)

    def test_map_basics_4(self):
        h = self.Map()
        h1 = h.set('key', [])
        h2 = h1.set('key', [])
        TestCase().assertIsNot(h1, h2)
        TestCase().assertEqual(len(h1), 1)
        TestCase().assertEqual(len(h2), 1)
        TestCase().assertIsNot(h1.get('key'), h2.get('key'))
    ...
    def test_map_collision_1(self):
        ...
        TestCase().assertEqual(h.get(k1), None)
        TestCase().assertEqual(h.get(k2), None)

        TestCase().assertEqual(h2.get(k1), 'a')
        TestCase().assertEqual(h2.get(k2), None)

        TestCase().assertEqual(h3.get(k1), 'a')
        TestCase().assertEqual(h3.get(k2), 'b')
        ...
        TestCase().assertEqual(h3.get(k1), 'a')
        TestCase().assertEqual(h3.get(k2), 'b')
        TestCase().assertEqual(h4.get(k1), 'a')
        TestCase().assertEqual(h4.get(k2), 'cc')
        TestCase().assertEqual(h4.get(k3), None)
        TestCase().assertEqual(h5.get(k1), 'a')
        TestCase().assertEqual(h5.get(k2), 'cc')
        TestCase().assertEqual(h5.get(k2), 'cc')
        TestCase().assertEqual(h5.get(k3), 'aa')

        TestCase().assertEqual(len(h), 0)
        TestCase().assertEqual(len(h2), 1)
        TestCase().assertEqual(len(h3), 2)
        TestCase().assertEqual(len(h4), 2)
        TestCase().assertEqual(len(h5), 3)
    ...
    def test_map_stress_01(self):
        ...
        for _ in range(RUN_XTIMES):
            ...
            for i in range(COLLECTION_SIZE):
                ...
                if not (i % CRASH_HASH_EVERY):
                    with HashKeyCrasher(error_on_hash=True):
                        with TestCase().assertRaises(HashingError):
                            h.set(key, i)
                ...
                if not (i % CRASH_EQ_EVERY):
                    with HashKeyCrasher(error_on_eq=True):
                        with TestCase().assertRaises(EqError):
                            h.get(KeyStr(i))  # really trigger __eq__

                d[key] = i
                TestCase().assertEqual(len(d), len(h))

                if not (i % TEST_ITERS_EVERY):
                    TestCase().assertEqual(set(h.items()), set(d.items()))
                    TestCase().assertEqual(len(h.items()), len(d.items()))

            TestCase().assertEqual(len(h), COLLECTION_SIZE)

            for key in range(COLLECTION_SIZE):
                TestCase().assertEqual(h.get(KeyStr(key), 'not found'), key)
            ...
            for iter_i, i in enumerate(keys_to_delete):
                ...
                h = h.delete(key)
                TestCase().assertEqual(h.get(key, 'not found'), 'not found')
                del d[key]
                TestCase().assertEqual(len(d), len(h))
                ...
                if not (iter_i % TEST_ITERS_EVERY):
                    TestCase().assertEqual(set(h.keys()), set(d.keys()))
                    TestCase().assertEqual(len(h.keys()), len(d.keys()))

            TestCase().assertEqual(len(d), 0)
            TestCase().assertEqual(len(h), 0)

            # ============

            for key in dm:
                TestCase().assertEqual(hm.get(str(key)), dm[key])
            TestCase().assertEqual(len(dm), len(hm))

            for i, key in enumerate(keys_to_delete):
                if str(key) in dm:
                    hm = hm.delete(str(key))
                    dm.pop(str(key))
                TestCase().assertEqual(hm.get(str(key), 'not found'), 'not found')
                TestCase().assertEqual(len(d), len(h))

                if not (i % TEST_ITERS_EVERY):
                    TestCase().assertEqual(set(h.values()), set(d.values()))
                    TestCase().assertEqual(len(h.values()), len(d.values()))

            TestCase().assertEqual(len(d), 0)
            TestCase().assertEqual(len(h), 0)
            TestCase().assertEqual(list(h.items()), [])
    ...
    def test_map_collision_3(self):
        ...
        TestCase().assertEqual({k.name for k in h.keys()}, {'C', 'D', 'E'})
    ...
    def test_map_stress_02(self):
        ...
        with h.mutate() as m:
            for i in range(COLLECTION_SIZE // 2, COLLECTION_SIZE):
                ...
                d[key] = i
                TestCase().assertEqual(len(d), len(m))
        ...
        TestCase().assertEqual(len(h), len(d))
        TestCase().assertEqual(set(h.items()), set(d.items()))
        ...
        with h.mutate() as m:
            for key in list(d):
                del d[key]
                del m[key]
                TestCase().assertEqual(len(m), len(d))
            h = m.finish()
    ...
    def test_map_delete_1(self):
        ...
        h = h.delete(C)
        TestCase().assertEqual(len(h), orig_len - 1)

        with TestCase().assertRaisesRegex(ValueError, 'cannot compare'):
            h.delete(Er)

        h = h.delete(D)
        TestCase().assertEqual(len(h), orig_len - 2)

        with TestCase().assertRaises(KeyError) as ex:
            h.delete(Z)
        TestCase().assertIs(ex.exception.args[0], Z)

        h = h.delete(A)
        TestCase().assertEqual(len(h), orig_len - 3)

        TestCase().assertEqual(h.get(A, 42), 42)
        TestCase().assertEqual(h.get(B), 'b')
        TestCase().assertEqual(h.get(E), 'e')
    ...
    def test_map_delete_2(self):
        ...
        with TestCase().assertRaises(KeyError) as ex:
            h.delete(Z)
        TestCase().assertIs(ex.exception.args[0], Z)
        TestCase().assertEqual(len(h), orig_len)
        ...
        TestCase().assertEqual(h.get(D), 'd')
        TestCase().assertEqual(h.get(E), 'e')
    ...
    def test_map_delete_3(self):
        ...
        TestCase().assertEqual(h.get(C), 'c')
        TestCase().assertEqual(h.get(B), 'b')

        h2 = h.delete(C)
        TestCase().assertEqual(len(h2), orig_len - 3)
    ...
    def test_map_delete_4(self):
        ...
        h = h.delete(A)
        TestCase().assertEqual(len(h), orig_len - 4)
    ...
    def test_map_delete_5(self):
        ...
        TestCase().assertEqual(len(h), 18)

        h = h.delete(keys[2])
        TestCase().assertEqual(len(h), 17)

        h = h.delete(collision_key16)
        TestCase().assertEqual(len(h), 16)
        h = h.delete(keys[16])
        TestCase().assertEqual(len(h), 15)

        h = h.delete(keys[1])
        TestCase().assertEqual(len(h), 14)
        with TestCase().assertRaises(KeyError) as ex:
            h.delete(keys[1])
        TestCase().assertIs(ex.exception.args[0], keys[1])
        TestCase().assertEqual(len(h), 14)
    ...
    def test_map_delete_6(self):
        ...
        h = h.delete(1)
        TestCase().assertEqual(len(h), 0)
        TestCase().assertEqual(h, self.Map())
    ...
    def test_map_items_3(self):
        h = self.Map()
        TestCase().assertEqual(len(h.items()), 0)
        TestCase().assertEqual(list(h.items()), [])

    def test_map_items_4(self):
        h = self.Map(a=1, b=2, c=3)
        k = h.items()
        TestCase().assertEqual(set(k), {('a', 1), ('b', 2), ('c', 3)})
        TestCase().assertEqual(set(k), {('a', 1), ('b', 2), ('c', 3)})
    ...
    def test_map_keys_1(self):
        ...
        TestCase().assertEqual(set(list(h.keys())), {A, B, C, D, E, F})
        TestCase().assertEqual(set(list(h)), {A, B, C, D, E, F})

    def test_map_keys_2(self):
        h = self.Map(a=1, b=2, c=3)
        k = h.keys()
        TestCase().assertEqual(set(k), {'a', 'b', 'c'})
        TestCase().assertEqual(set(k), {'a', 'b', 'c'})
    ...
    def test_map_values_1(self):
        ...
        TestCase().assertEqual(set(list(h.values())), {'a', 'b', 'c', 'd', 'e', 'f'})

    def test_map_values_2(self):
        h = self.Map(a=1, b=2, c=3)
        k = h.values()
        TestCase().assertEqual(set(k), {1, 2, 3})
        TestCase().assertEqual(set(k), {1, 2, 3})
    ...
    def test_map_eq_3(self):
        TestCase().assertNotEqual(self.Map(), 1)
    ...
    def test_map_in_1(self):
        ...
        TestCase().assertTrue(A in h)
        TestCase().assertFalse(B in h)
    ...
    def test_map_getitem_1(self):
        ...
        TestCase().assertEqual(h[A], 1)
        TestCase().assertEqual(h[AA], 1)
    ...
    def test_repr_1(self):
        h = self.Map()
        TestCase().assertEqual(repr(h), 'immutables.Map({})')

        h = h.set(1, 2).set(2, 3).set(3, 4)
        TestCase().assertEqual(repr(h), 'immutables.Map({1: 2, 2: 3, 3: 4})')

    def test_repr_2(self):
        h = self.Map()
        A = HashKey(100, 'A')

        with TestCase().assertRaises(ReprError):
    ...
    def test_repr_3(self):
        ...
        TestCase().assertEqual(repr(h), 'immutables.Map({{...}: 1})')

    def test_hash_1(self):
        h = self.Map()
        TestCase().assertNotEqual(hash(h), -1)
        TestCase().assertEqual(hash(h), hash(h))
    ...
    def test_abc_1(self):
        TestCase().assertTrue(issubclass(self.Map, collections.abc.Mapping))
    ...
    def test_map_mut_1(self):
        ...
        TestCase().assertFalse(isinstance(hm1, self.Map))

        TestCase().assertIsNot(hm1, hm2)
        TestCase().assertEqual(hm1['a'], 1)
        TestCase().assertEqual(hm2['a'], 1)
        ...
        TestCase().assertEqual(hm1['a'], 1)
        TestCase().assertEqual(hm1.get('x', -1), -1)

        TestCase().assertEqual(hm2['a'], 1000)
        TestCase().assertTrue('x' in hm2)

        h1 = hm1.finish()
        h2 = hm2.finish()

        TestCase().assertTrue(isinstance(h1, self.Map))

        TestCase().assertEqual(dict(h.items()), {'a': 1})
        TestCase().assertEqual(dict(h1.items()), {'a': 1, 'b': 2, 'c': 3})
        TestCase().assertEqual(dict(h2.items()), {'a': 1000, 'x': 100})
    ...
    def test_map_mut_2(self):
        ...
        TestCase().assertEqual(dict(h.items()), {'a': 1})
        TestCase().assertEqual(dict(h2.items()), {'a': 4})
    ...
    def test_map_mut_3(self):
        ...
        TestCase().assertEqual(repr(hm1), "immutables.MapMutation({'a': 1})")

        with TestCase().assertRaisesRegex(TypeError, 'unhashable type'):
            hash(hm1)
    ...
    def test_map_mut_4(self):
        ...
        TestCase().assertEqual(hm1, hm2)

        hm1.set('a', 10)
        TestCase().assertNotEqual(hm1, hm2)

        hm2.set('a', 10)
        TestCase().assertEqual(hm1, hm2)

        TestCase().assertEqual(hm2.pop('a'), 10)
        TestCase().assertNotEqual(hm1, hm2)

    def test_map_mut_5(self):
        h = self.Map({'a': 1, 'b': 2}, z=100)
        TestCase().assertTrue(isinstance(h, self.Map))
        TestCase().assertEqual(dict(h.items()), {'a': 1, 'b': 2, 'z': 100})

        h2 = h.update(z=200, y=-1)
        TestCase().assertEqual(dict(h.items()), {'a': 1, 'b': 2, 'z': 100})
        TestCase().assertEqual(dict(h2.items()), {'a': 1, 'b': 2, 'z': 200, 'y': -1})

        h3 = h2.update([(1, 2), (3, 4)])
        TestCase().assertEqual(dict(h.items()), {'a': 1, 'b': 2, 'z': 100})
        TestCase().assertEqual(dict(h2.items()), {'a': 1, 'b': 2, 'z': 200, 'y': -1})
        TestCase().assertEqual(dict(h3.items()),
                         {'a': 1, 'b': 2, 'z': 200, 'y': -1, 1: 2, 3: 4})

        h4 = h3.update()
        TestCase().assertIs(h4, h3)

        h5 = h4.update(self.Map({'zzz': 'yyz'}))

        TestCase().assertEqual(dict(h5.items()),
                         {'a': 1, 'b': 2, 'z': 200, 'y': -1, 1: 2, 3: 4,
                          'zzz': 'yyz'})
    ...
    def test_map_mut_6(self):
        ...
        with TestCase().assertRaisesRegex(TypeError, 'not iterable'):
            h.update(1)

        with TestCase().assertRaisesRegex(ValueError, 'map update sequence element'):
            h.update([(1, 2), (3, 4, 5)])

        with TestCase().assertRaisesRegex(TypeError, 'cannot convert map update'):
            h.update([(1, 2), 1])
    ...
    def test_map_mut_8(self):
        ...
        h = self.Map({key1: 123})
        TestCase().assertEqual(dict(h.items()), {key1: 123})
    ...
    def test_map_mut_11(self):
        ...
        mm = m.mutate()
        TestCase().assertEqual(mm.pop('a', 1), 1)
        TestCase().assertEqual(mm.finish(), self.Map({'b': 2}))

        mm = m.mutate()
        TestCase().assertEqual(mm.pop('b', 1), 2)
        TestCase().assertEqual(mm.finish(), self.Map({'a': 1}))

        mm = m.mutate()
        TestCase().assertEqual(mm.pop('b', 1), 2)
        del mm['a']
        TestCase().assertEqual(mm.finish(), self.Map())
    ...
    def test_map_mut_12(self):
        ...
        with TestCase().assertRaisesRegex(ValueError, 'has been finished'):
            mm.pop('a')
    ...
    def test_map_mut_14(self):
        ...
        TestCase().assertEqual(mm.finish(), self.Map(z=100, b=2))
    ...
    def test_map_mut_15(self):
        ...
        TestCase().assertEqual(mm.finish(), self.Map(z=100, b=2))
        TestCase().assertEqual(m, self.Map(a=1, b=2))
    ...
    def test_map_mut_16(self):
        ...
        TestCase().assertEqual(m, m2)
        TestCase().assertEqual(len(m), len(m2))
        TestCase().assertEqual(hash(m), hash(m2))

        TestCase().assertIsNot(m, m2)
        TestCase().assertEqual(m3, self.Map(a=1, b=2, c=3))
    ...
    def test_map_mut_18(self):
        ...
        TestCase().assertEqual(len(m2), 6)
        TestCase().assertEqual(m2, expected)
        TestCase().assertEqual(m, self.Map(a=1, b=2))

    def test_map_mut_19(self):
        m = self.Map(a=1, b=2)
        m2 = m.update({'a': 20})
        TestCase().assertEqual(len(m2), 2)
        ...
        for _ in range(RUN_XTIMES):
            ...
            for i in range(COLLECTION_SIZE // TEST_ITERS_EVERY):
                ...
                for j in range(TEST_ITERS_EVERY):
                    ...
                    TestCase().assertEqual(len(hm), len(d))

                h2 = hm.finish()
                TestCase().assertEqual(dict(h2.items()), d)
                h = h2

            TestCase().assertEqual(dict(h.items()), d)
            TestCase().assertEqual(len(h), len(d))
    ...
    def test_map_pickle(self):
        ...
        for proto in range(pickle.HIGHEST_PROTOCOL):
            ...
            TestCase().assertTrue(isinstance(uh, self.Map))
            TestCase().assertEqual(h, uh)

        with TestCase().assertRaisesRegex(TypeError, "can('t|not) pickle"):
            pickle.dumps(h.mutate())

    def test_map_is_subscriptable(self):
        if sys.version_info >= (3, 9):
            with_args = self.Map[int, str]
            TestCase().assertIs(with_args.__origin__, self.Map)
            TestCase().assertEqual(with_args.__args__, (int, str))
        else:
            TestCase().assertIs(self.Map[int, str], self.Map)

    def test_kwarg_named_col(self):
        TestCase().assertEqual(dict(self.Map(col=0)), {"col": 0})
        TestCase().assertEqual(dict(self.Map(a=0, col=1)), {"a": 0, "col": 1})
        TestCase().assertEqual(dict(self.Map({"a": 0}, col=1)), {"a": 0, "col": 1})
